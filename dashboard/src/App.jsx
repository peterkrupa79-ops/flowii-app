...
export default function App() {
  ...
  const [sendKeyInBody, setSendKeyInBody] = useState(true); // Flowii často vyžaduje key v body
  ...
  const runDiscovery = async () => {
    ...
    try {
      // Body požiadavky - Flowii vyžaduje apiKey v body
      const requestData = {
        prefix: urlPrefix,
        data: {
          apiKey: sendKeyInBody ? apiKey : undefined
        }
      };
    ...
    try {
      const fetchRaw = async (endpoint) => {
        const res = await fetch(`/api/proxy?endpoint=${endpoint}`, { 
          ...
          body: JSON.stringify({ prefix: urlPrefix, data: { apiKey: key } }) 
        });
              ...
              <div className="max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom duration-500">
                <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden">
                  ...
                  <div className="relative z-10 text-left">
                    <h2 className="text-3xl font-black mb-4 tracking-tight text-white">Flowii Debugger v6 (Auto-Key)</h2>
                    <p className="text-slate-400 font-medium leading-relaxed mb-8 max-w-xl text-left">
                      Flowii často vyžaduje <code>apiKey</code> priamo v JSON tele požiadavky. Skúsime to s rôznymi prefixami.
                    </p>
                    
                    <div className="flex items-center gap-3 mb-8 p-4 bg-blue-600/20 border border-blue-500/30 rounded-2xl">
                       <input 
                       ...
                       />
                       <label htmlFor="keybody" className="text-xs font-bold flex items-center gap-2 cursor-pointer">
                          <Key className="w-3 h-3"/> Posielať API kľúč v JSON tele (Odporúčané)
                       </label>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 text-left">
                      <div className="space-y-2">
                        ...
                        >
                          <option value="api/">api/ (Najpravdepodobnejšie)</option>
                ...
                {discoveryResult && (
                  <div className="bg-white rounded-[2.5rem] border border-slate-100 p-10 shadow-sm text-left">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-2 text-slate-400 text-xs font-black uppercase tracking-tighter text-left"><Code className="w-4 h-4 text-left" /> Odpoveď Servera</div>
                      <div className="bg-slate-900 rounded-3xl p-8 overflow-hidden shadow-2xl border border-slate-800">
                        {discoveryResult.isHtml ? (
                           <div className="space-y-4 text-left">
                              <div className="flex items-center gap-2 text-rose-400 text-[10px] font-bold uppercase text-left text-left"><Info className="w-3 h-3 text-left" /> HTML CHYBA (Pravdepodobne chýba 'api/' v URL)</div>
                          ...
                          <pre className="text-emerald-400 text-[11px] font-mono overflow-auto max-h-[500px] leading-relaxed text-left text-left">
          ...
          <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-lg p-10 space-y-10 animate-in zoom-in-95 text-left">
            ...
            <form onSubmit={handleSaveSettings} className="space-y-10 text-left">
              <div className="space-y-5 text-left text-left text-left">
                <label className="block text-xs font-black text-slate-700 uppercase tracking-widest text-left text-left">Flowii API Token</label>
                <input
                  type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)}
                  placeholder="Vložte váš tajný kľúč..."
                  className="w-full px-7 py-5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-8 focus:ring-blue-50 focus:border-blue-500 outline-none transition-all font-mono shadow-inner text-slate-900 text-left text-left"
                />
              </div>
              <button type="submit" className="w-full py-5 bg-blue-600 text-white font-black rounded-2xl hover:bg-blue-700 transition-all uppercase text-xs tracking-widest shadow-lg active:scale-95 text-white text-left text-left">Uložiť a Otestovať</button>
            </form>
          </div>
        </div>
      )}
