...
export default function App() {
  ...
  // Discovery state (v12 - God Mode)
  const [urlBase, setUrlBase] = useState('https://api.flowii.com'); // Niektoré účty chcú crm.flowii.com
  ...
  const runDiscovery = async () => {
    ...
    try {
      const response = await fetch(`/api/proxy?endpoint=${sanitizedEndpoint}`, {
        ...
        headers: headers,
        body: JSON.stringify({
          base: urlBase, // Povieme proxy, na akú doménu má ísť
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans text-slate-800">
      ...
      <aside className={`fixed inset-y-0 left-0 bg-slate-900 text-slate-300 w-72 transform transition-transform duration-500 ease-in-out z-40 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:static md:block border-r border-slate-800 shadow-2xl text-left`}>
        ...
        <nav className="mt-8 px-6 space-y-2">
          ...
          <button onClick={() => setActiveTab('discovery')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl transition-all ${activeTab === 'discovery' ? 'bg-blue-600 text-white shadow-xl' : 'hover:bg-slate-800 text-slate-400'}`}>
            ...
            <span className="font-bold tracking-tight">Debugger (v12)</span>
          </button>
        </nav>
            ...
            <div className="max-w-5xl mx-auto space-y-8 animate-in slide-in-from-bottom duration-500 text-left">
              <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden text-left">
                ...
                <div className="relative z-10 text-left">
                  <h2 className="text-3xl font-black mb-4 tracking-tight text-white text-left">God Mode Debugger v12</h2>
                  <p className="text-slate-400 font-medium leading-relaxed mb-8 max-w-2xl text-left">
                    IIS Server stále hlási 404. Skúsime zmeniť cieľovú doménu a metódu autentifikácie.
                  </p>
                  
                  <div className="flex flex-wrap gap-3 mb-10 text-left">
                     <button 
                       onClick={() => { setUrlBase('https://crm.flowii.com'); setUrlPrefix('api/v1/'); setDiscoveryEndpoint('user/me'); setHttpMethod('GET'); }}
                       className="px-5 py-2.5 bg-amber-600/20 border border-amber-500/30 rounded-xl text-[10px] font-black hover:bg-amber-600 transition-all flex items-center gap-2"
                     >
                        <Server className="w-4 h-4"/> Alternatívna Doména (crm.flowii.com)
                     </button>
                     <button 
                       onClick={() => { setDiscoveryEndpoint('auth/login'); setHttpMethod('POST'); setUrlPrefix('api/v1/'); }}
                       className="px-5 py-2.5 bg-emerald-600/20 border border-emerald-500/30 rounded-xl text-[10px] font-black hover:bg-emerald-600 transition-all flex items-center gap-2"
                     >
                        <Lock className="w-4 h-4"/> Test Prihlásenia (Login)
                     </button>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8 text-left text-left">
                    <div className="space-y-2 text-left">
                      <label className="text-[10px] font-black uppercase text-slate-500 text-left">Cieľová Doména</label>
                      <select value={urlBase} onChange={(e) => setUrlBase(e.target.value)} className="w-full bg-slate-800 border-2 border-slate-700 rounded-xl p-3 text-[11px] font-bold text-white outline-none focus:border-blue-500 text-left">
                        <option value="https://api.flowii.com">api.flowii.com</option>
                        <option value="https://crm.flowii.com">crm.flowii.com</option>
                      </select>
                    </div>
                    <div className="space-y-2 text-left">
                      <label className="text-[10px] font-black uppercase text-slate-500 text-left">Prefix</label>
                      <select value={urlPrefix} onChange={(e) => setUrlPrefix(e.target.value)} className="w-full bg-slate-800 border-2 border-slate-700 rounded-xl p-3 text-[11px] font-bold text-white outline-none focus:border-blue-500 text-left text-left">
                    ...
                    <div className="space-y-2 text-left text-left text-left">
                      <label className="text-[10px] font-black uppercase text-slate-500 text-left text-left text-left">Hlavička</label>
                      <select value={authStyle} onChange={(e) => setAuthStyle(e.target.value)} className="w-full bg-slate-800 border-2 border-slate-700 rounded-xl p-3 text-[11px] font-bold text-white outline-none focus:border-blue-500 text-left text-left text-left">
                    ...
                    <div className="space-y-2 text-left text-left text-left text-left">
                      <label className="text-[10px] font-black uppercase text-slate-500 text-left text-left text-left text-left">Metóda</label>
                      <select value={httpMethod} onChange={(e) => setHttpMethod(e.target.value)} className="w-full bg-slate-800 border-2 border-slate-700 rounded-xl p-3 text-[11px] font-bold text-white outline-none focus:border-blue-500 text-left text-left text-left text-left">
                        <option value="POST">POST</option>
                        <option value="GET">GET</option>
                      </select>
                    </div>
                    <div className="space-y-2 text-left text-left text-left text-left text-left">
                      <label className="text-[10px] font-black uppercase text-slate-500 text-left text-left text-left text-left text-left">Endpoint</label>
                      <input type="text" value={discoveryEndpoint} onChange={(e) => setDiscoveryEndpoint(e.target.value)} className="w-full bg-slate-800 border-2 border-slate-700 rounded-xl p-3 text-[11px] font-bold text-white outline-none focus:border-blue-500 text-left text-left text-left text-left text-left" />
                    </div>
                  </div>

                  <button 
                    onClick={runDiscovery} disabled={discoveryLoading}
                    className="w-full bg-blue-600 hover:bg-blue-50 disabled:opacity-50 py-4 rounded-2xl font-black flex items-center justify-center gap-3 shadow-lg active:scale-95 text-white uppercase text-xs tracking-widest text-left"
                  >
                    {discoveryLoading ? <RefreshCw className="w-5 h-5 animate-spin text-white" /> : <Send className="w-5 h-5 text-white" />}
                    Vykonať Hĺbkovú Analýzu
                  </button>
                </div>
              </div>
              ...
              {discoveryResult && (
                <div className="bg-white rounded-[2.5rem] border border-slate-100 p-10 shadow-sm text-left">
                  ...
                  {discoveryResult.isHtml && (
                    <div className="mb-8 p-6 bg-rose-50 border border-rose-100 rounded-3xl flex gap-5 items-start text-left">
                        <ShieldAlert className="w-8 h-8 text-rose-500 shrink-0" />
                        <div className="text-left text-left">
                            <h4 className="text-rose-900 font-bold mb-1 text-left text-left">Pretrvávajúca 404-ka</h4>
                            <p className="text-rose-700 text-xs leading-relaxed text-left text-left">
                                IIS Server stále odmieta túto adresu. Skúste kliknúť na tlačidlo <b>"Alternatívna Doména"</b> vyššie a zmeňte hlavičku na <b>"Api-Key"</b>. 
                                Ak to nepomôže, skontrolujte, či váš API kľúč nie je viazaný na IP adresu servera.
                            </p>
                        </div>
                    </div>
                  )}
                  ...
                  <div className="space-y-4 text-left text-left text-left">
                    <div className="flex items-center gap-2 text-slate-400 text-xs font-black uppercase tracking-tighter text-left text-left text-left text-left"><Code className="w-4 h-4 text-left text-left text-left text-left" /> Raw Output</div>
                    <div className="bg-slate-900 rounded-3xl p-8 overflow-hidden shadow-2xl border border-slate-800 text-left">
                      {discoveryResult.isHtml ? (
                         <div className="space-y-4 text-left text-left text-left text-left">
                            <div className="flex items-center gap-2 text-rose-400 text-[10px] font-bold uppercase text-left text-left text-left text-left text-left text-left text-left"><Info className="w-3 h-3 text-left" /> HTML 404 (Server zamietol kombináciu)</div>
                            <div className="text-slate-500 text-[10px] font-mono break-all opacity-50 text-left text-left text-left text-left text-left">{discoveryResult.payload?.raw}</div>
                         </div>
                      ) : (
                        <pre className="text-emerald-400 text-[11px] font-mono overflow-auto max-h-[500px] leading-relaxed text-left text-left text-left text-left text-left">
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center z-50 p-6 animate-in fade-in duration-300">
          <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-lg p-10 space-y-10 animate-in zoom-in-95 text-left text-left text-left text-left text-left">
            <div className="flex items-center justify-between text-left text-left text-left text-left text-left text-left">
              <h3 className="text-2xl font-black text-slate-900 tracking-tight text-left text-left text-left text-left text-left text-left">Nastavenia API</h3>
              <button onClick={() => setShowSettings(false)} className="p-3.5 hover:bg-slate-50 rounded-2xl border border-slate-100 text-slate-400 text-left text-left text-left text-left text-left text-left text-left"><X className="text-left text-slate-400" /></button>
            </div>
            <form onSubmit={handleSaveSettings} className="space-y-10 text-left text-left text-left text-left text-left text-left text-left text-left">
              <div className="space-y-5 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
                <label className="block text-xs font-black text-slate-700 uppercase tracking-widest text-left text-left text-left text-left text-left text-left">Flowii API Token</label>
                <input
                  type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)}
                  placeholder="Vložte váš tajný kľúč..."
                  className="w-full px-7 py-5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-8 focus:ring-blue-50 focus:border-blue-500 outline-none transition-all font-mono shadow-inner text-slate-900 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left"
                />
              </div>
              <button type="submit" className="w-full py-5 bg-blue-600 text-white font-black rounded-2xl hover:bg-blue-700 transition-all uppercase text-xs tracking-widest shadow-lg active:scale-95 text-white text-left text-left text-left text-left text-left text-left text-left text-left text-left">Uložiť a Synchronizovať</button>
            </form>
          </div>
        </div>
      )}
