...
export default function App() {
  ...
  // Debugger state (v8 - Optimized for Apiary Docs)
  ...
  // Pomocná funkcia na čistenie kľúča od ne-ASCII znakov (napr. symbol €)
  ...
  /**
   * Spustenie testu pripojenia (v8 Master Debugger)
   */
    ...
    try {
      const response = await fetch(`/api/proxy?endpoint=${discoveryEndpoint}`, {
        ...
        headers: {
          ...
          'Authorization': `Api-Key ${cleanKey}` // Skúšame Api-Key hlavičku podľa manuálu
        },
        body: JSON.stringify({
          prefix: urlPrefix,
          data: {} // Povinné prázdne body pre Flowii POST zoznamy
        })
      });
    ...
    try {
      const fetchRaw = async (endpoint) => {
        const res = await fetch(`/api/proxy?endpoint=${endpoint}`, { 
          method: 'POST', 
          headers: { 'Authorization': `Api-Key ${cleanKey}`, 'Content-Type': 'application/json' }, 
          body: JSON.stringify({ prefix: 'api/v1/', data: {} }) 
        });
      ...
      setDashboardData({
        revenue: invList.reduce((acc, inv) => acc + (parseFloat(inv.totalPrice) || 0), 0),
        activePartners: (partners.data || []).length,
        openDeals: 0,
        unpaidInvoices: invList.filter(inv => inv.paymentStatus !== 'paid').length,
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans text-slate-800">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden text-left text-slate-900">
        ...
        <div className="flex-1 overflow-auto p-8 lg:p-12 space-y-12">
          {error && (
            <div className="bg-rose-50 border-l-4 border-rose-500 p-6 rounded-r-3xl flex items-start gap-6 shadow-sm text-left animate-in slide-in-from-top">
            ...
            {activeTab === 'dashboard' && (
              <div className="space-y-12 text-left">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 text-left">
                  ...
                  <StatCard title="Otvorené obchody" value={dashboardData.openDeals} icon={Briefcase} />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                  ...
                  <div className="h-[450px]"><SimpleBarChart title="Obchodné prípady" data={dashboardData.monthlyDeals} /></div>
                </div>
              </div>
            )}
              ...
              <div className="max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom duration-500 text-left">
                <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden">
                  ...
                  <div className="relative z-10 text-left">
                    <h2 className="text-3xl font-black mb-4 tracking-tight text-white text-left">Flowii Debugger v8 (Apiary-Fix)</h2>
                    <p className="text-slate-400 font-medium leading-relaxed mb-8 max-w-xl">
                      Tento test používa hlavičku <code>Api-Key</code> a posiela prázdny JSON body <code>{"{}"}</code>. Skúste prefix <code>api/v1/</code>.
                    </p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 text-left text-left">
                      <div className="space-y-2">
                        ...
                        >
                          <option value="api/v1/">api/v1/ (Podľa dokumentácie)</option>
                    ...
                    >
                      ...
                      Otestovať presne podľa manuálu
                    </button>
                  </div>
                </div>
                  ...
                  <div className="bg-white rounded-[2.5rem] border border-slate-100 p-10 shadow-sm text-left">
                    <div className="flex items-center justify-between mb-8">
                      <div className="flex items-center gap-4 text-left">
                        ...
                        <div className="text-left">
                          <h3 className="text-xl font-bold text-slate-900 text-left">Výsledok Testu</h3>
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-2 text-slate-400 text-xs font-black uppercase tracking-tighter text-left text-left"><Code className="w-4 h-4 text-left text-left" /> Odpoveď zo servera</div>
                      <div className="bg-slate-900 rounded-3xl p-8 overflow-hidden shadow-2xl border border-slate-800">
                        {discoveryResult.isHtml ? (
                           <div className="space-y-4 text-left text-left">
                              <div className="flex items-center gap-2 text-rose-400 text-[10px] font-bold uppercase text-left text-left text-left text-left"><Info className="w-3 h-3 text-left" /> HTML CHYBA (Zlá cesta)</div>
                              <div className="text-slate-500 text-[10px] font-mono break-all opacity-50 text-left text-left">{discoveryResult.payload?.raw}</div>
                           </div>
                        ) : (
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center z-50 p-6 animate-in fade-in duration-300">
          <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-lg p-10 space-y-10 animate-in zoom-in-95 text-left text-left">
            <div className="flex items-center justify-between text-left text-left text-left text-left">
              ...
              <button onClick={() => setShowSettings(false)} className="p-3.5 hover:bg-slate-50 rounded-2xl border border-slate-100 text-slate-400 text-left text-left text-left text-left text-left text-left"><X className="text-left text-slate-400" /></button>
            </div>
            <form onSubmit={handleSaveSettings} className="space-y-10 text-left text-left text-left text-left text-left text-left text-left text-left">
              <div className="space-y-5 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
                <label className="block text-xs font-black text-slate-700 uppercase tracking-widest text-left text-left text-left text-left text-left text-left text-left">Flowii API Token</label>
                <input
                  type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)}
                  placeholder="Vložte váš tajný kľúč..."
                  className="w-full px-7 py-5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-8 focus:ring-blue-50 focus:border-blue-500 outline-none transition-all font-mono shadow-inner text-slate-900 text-left text-left text-left text-left text-left text-left text-left text-left text-left"
                />
                <div className="flex items-center gap-2 px-2 text-[10px] text-slate-400 font-bold italic text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
                  <ShieldCheck className="w-3 h-3 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left" /> Automaticky čistíme nepovolené znaky (ako €)
                </div>
              </div>
              ...
              <button type="button" onClick={() => fetchFlowiiData('demo-key')} className="w-full py-2 text-xs font-bold text-slate-400 hover:text-slate-600 transition-colors uppercase tracking-widest text-center text-left text-left text-left text-left text-left text-left text-left text-left text-left">Pokračovať v Demo režime</button>
            </form>
          </div>
        </div>
      )}
