...
const StatCard = ({ title, value, icon: Icon, trend, trendValue }) => (
  <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex items-start justify-between group hover:border-blue-300 transition-all text-left text-slate-900">
    ...
    <div className="p-4 bg-slate-50 text-blue-600 rounded-2xl group-hover:bg-blue-600 group-hover:text-white transition-all shadow-inner border border-slate-100 text-slate-900">
...
export default function App() {
  ...
  // Debug nastavenia pre Flowii (v6)
  ...
  const [sendKeyInBody, setSendKeyInBody] = useState(true); 
  ...
  // Pomocná funkcia na čistenie kľúča od ne-ASCII znakov (prevencia ByteString chyby)
  const getCleanApiKey = (key) => {
    if (!key) return "";
    // Odstráni všetko, čo nie je v rozsahu ASCII 0-127 (napr. symbol €)
  ...
  const runDiscovery = async () => {
    ...
    try {
      const requestData = {
        prefix: urlPrefix,
        data: {
          apiKey: sendKeyInBody ? cleanKey : undefined
        }
      };
      ...
      setDiscoveryResult({
        ...
        keyInBody: sendKeyInBody ? "ÁNO" : "NIE"
      });
    ...
    try {
      const fetchRaw = async (endpoint) => {
        const res = await fetch(`/api/proxy?endpoint=${endpoint}`, { 
          ...
          body: JSON.stringify({ prefix: urlPrefix, data: { apiKey: cleanKey } }) 
        });
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans text-slate-800">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden text-left text-slate-900">
        <header className="bg-white border-b border-slate-100 h-20 flex items-center justify-between px-8 shrink-0">
          ...
          <button onClick={() => fetchFlowiiData(apiKey)} className="p-2 bg-slate-100 rounded-xl text-slate-600 hover:bg-blue-50 transition-colors shadow-sm text-left">
              ...
              <div className="max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom duration-500 text-left">
                <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden text-left">
                  ...
                  <div className="relative z-10 text-left">
                    <h2 className="text-3xl font-black mb-4 tracking-tight text-white text-left">Flowii Debugger v6 (Auto-Sanitize)</h2>
                    <p className="text-slate-400 font-medium leading-relaxed mb-8 max-w-xl text-left">
                      Tento nástroj automaticky odstraňuje nepovolené znaky (ako €) z hlavičky <code>Authorization</code>, aby sa predišlo chybe ByteString.
                    </p>
                    
                    <div className="flex items-center gap-3 mb-8 p-4 bg-blue-600/20 border border-blue-500/30 rounded-2xl text-left">
                       <input 
                         type="checkbox" checked={sendKeyInBody} onChange={(e) => setSendKeyInBody(e.target.checked)}
                         className="w-5 h-5 accent-blue-500" id="keybody"
                       />
                       <label htmlFor="keybody" className="text-xs font-bold flex items-center gap-2 cursor-pointer text-white">
                          <Key className="w-3 h-3 text-white"/> Posielať API kľúč v JSON tele (Odporúčané)
                       </label>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 text-left">
                      <div className="space-y-2 text-left">
                        ...
                        >
                          ...
                          <option value="api/v1/">api/v1/ (Starší)</option>
                ...
                {discoveryResult && (
                  <div className="bg-white rounded-[2.5rem] border border-slate-100 p-10 shadow-sm text-left">
                    <div className="flex items-center justify-between mb-8 text-left">
                      <div className="flex items-center gap-4 text-left text-left text-left text-left">
                        ...
                        <div className="text-left text-left text-left text-left text-left">
                          <h3 className="text-xl font-bold text-left text-left text-left text-left text-left text-slate-900">Analýza Odpovede</h3>
                    ...
                    <div className="space-y-4 text-left text-left text-left text-left">
                      <div className="flex items-center gap-2 text-slate-400 text-xs font-black uppercase tracking-tighter text-left text-left text-left text-left text-left text-left text-left text-left text-left"><Code className="w-4 h-4 text-left text-left text-left text-left text-left text-left" /> Výstup zo Servera</div>
                      <div className="bg-slate-900 rounded-3xl p-8 overflow-hidden shadow-2xl text-left border border-slate-800">
                        {discoveryResult.isHtml ? (
                           <div className="space-y-4 text-left text-left">
                              <div className="flex items-center gap-2 text-rose-400 text-[10px] font-bold uppercase text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left"><Info className="w-3 h-3 text-left" /> HTML CHYBA (Chybná URL alebo neautorizovaný prístup)</div>
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center z-50 p-6 animate-in fade-in duration-300">
          <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-lg p-10 space-y-10 animate-in zoom-in-95 text-left text-left text-left text-left">
            ...
            <form onSubmit={handleSaveSettings} className="space-y-10 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
              <div className="space-y-5 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
                ...
                />
                <div className="flex items-center gap-2 px-2 text-[10px] text-slate-400 font-bold italic text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
                  <ShieldCheck className="w-3 h-3 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left" /> Zabezpečené a prečistené pred odoslaním
                </div>
              </div>
              <button type="submit" className="w-full py-5 bg-blue-600 text-white font-black rounded-2xl hover:bg-blue-700 transition-all uppercase text-xs tracking-widest shadow-lg active:scale-95 text-white text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">Uložiť a Otestovať</button>
            </form>
          </div>
        </div>
      )}
