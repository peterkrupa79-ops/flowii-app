...
export default function App() {
  ...
  // Discovery state (v13 - Emergency Protocol)
  ...
  const runDiscovery = async () => {
    ...
    // Kritické prepínanie hlavičiek pre IIS
    ...
    // Ak je 'None', neposielame žiadnu hlavičku (kľúč pôjde len do body)
    ...
    try {
      const response = await fetch(`/api/proxy?endpoint=${sanitizedEndpoint}`, {
        ...
        body: JSON.stringify({
          ...
          method: httpMethod,
          data: { 
            apiKey: cleanKey, // Väčšina Flowii volaní chce kľúč aj tu
            user_ticket: cleanKey // Niektoré staršie verzie používajú kľúč ako ticket
          }
        })
      });
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans text-slate-800">
      ...
      <aside className={`fixed inset-y-0 left-0 bg-slate-900 text-slate-300 w-72 transform transition-transform duration-500 ease-in-out z-40 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:static md:block border-r border-slate-800 shadow-2xl text-left`}>
        ...
        <nav className="mt-8 px-6 space-y-2">
          ...
          <button onClick={() => setActiveTab('discovery')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl transition-all ${activeTab === 'discovery' ? 'bg-blue-600 text-white shadow-xl' : 'hover:bg-slate-800 text-slate-400'}`}>
            ...
            <span className="font-bold tracking-tight">Debugger (v13)</span>
          </button>
        </nav>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden text-left text-slate-900">
        <header className="bg-white border-b border-slate-100 h-20 flex items-center justify-between px-8 shrink-0">
          <div className="flex items-center gap-4 text-left">
            ...
            <h1 className="text-2xl font-black text-slate-900 tracking-tight text-left">{activeTab === 'dashboard' ? 'Dashboard' : 'Emergency Debugger'}</h1>
          </div>
        ...
        <div className="flex-1 overflow-auto p-8 lg:p-12 space-y-12 text-left">
          {activeTab === 'dashboard' ? (
            ...
            <div className="max-w-5xl mx-auto space-y-8 animate-in slide-in-from-bottom duration-500 text-left">
              ...
              {/* EMERGENCY TIPS */}
              <div className="bg-blue-600 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden text-left border-4 border-white">
                <div className="absolute top-0 right-0 p-12 opacity-10"><LifeBuoy className="w-32 h-32 text-white" /></div>
                <div className="relative z-10">
                  <h2 className="text-3xl font-black mb-4 tracking-tight text-white text-left">Emergency Protokol v13</h2>
                  <p className="text-blue-100 font-bold leading-relaxed mb-8 max-w-2xl text-left">
                    Ak stále vidíte 404, musíme zistiť, či server Flowii vôbec akceptuje váš kľúč. Skúste tieto tri kroky v poradí:
                  </p>
                  
                  <div className="flex flex-wrap gap-3 text-left">
                     <button 
                       onClick={() => { setDiscoveryEndpoint('auth/login'); setHttpMethod('POST'); setUrlPrefix('api/v1/'); setAuthStyle('None'); }}
                       className="px-5 py-3 bg-white text-blue-600 rounded-2xl text-xs font-black hover:bg-blue-50 transition-all flex items-center gap-2 shadow-xl"
                     >
                        <Lock className="w-4 h-4"/> 1. TEST PRIHLÁSENIA (Najdôležitejší)
                     </button>
                     <button 
                       onClick={() => { setAuthStyle('None'); setHttpMethod('POST'); }}
                       className="px-5 py-3 bg-blue-500 text-white border-2 border-blue-400 rounded-2xl text-xs font-black hover:bg-blue-400 transition-all flex items-center gap-2"
                     >
                        <ShieldAlert className="w-4 h-4"/> 2. BEZ HLAVIČIEK (Iba Body)
                     </button>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-[2.5rem] p-10 border border-slate-200 shadow-sm text-left">
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8 text-left">
                  ...
                  <div className="space-y-2 text-left">
                    ...
                    <select value={urlPrefix} onChange={(e) => setUrlPrefix(e.target.value)} className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl p-3 text-[11px] font-bold outline-none focus:border-blue-500 text-left text-left">
                  ...
                  <div className="space-y-2 text-left">
                    ...
                    <select value={authStyle} onChange={(e) => setAuthStyle(e.target.value)} className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl p-3 text-[11px] font-bold outline-none focus:border-blue-500 text-left text-left text-left text-left">
                      ...
                      <option value="None">ŽIADNA (iba v tele)</option>
                    </select>
                  </div>
                  <div className="space-y-2 text-left">
                    <label className="text-[10px] font-black uppercase text-slate-400 text-left text-left text-left text-left text-left">Metóda</label>
                    <select value={httpMethod} onChange={(e) => setHttpMethod(e.target.value)} className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl p-3 text-[11px] font-bold outline-none focus:border-blue-500 text-left text-left text-left text-left text-left">
                      <option value="POST">POST</option>
                      <option value="GET">GET</option>
                    </select>
                  </div>
                  <div className="space-y-2 text-left text-left text-left text-left text-left text-left">
                    <label className="text-[10px] font-black uppercase text-slate-400 text-left text-left text-left text-left text-left text-left">Endpoint</label>
                    <input type="text" value={discoveryEndpoint} onChange={(e) => setDiscoveryEndpoint(e.target.value)} className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl p-3 text-[11px] font-bold outline-none focus:border-blue-500 text-left text-left text-left text-left text-left text-left" />
                  </div>
                </div>
              ...
              {discoveryResult && (
                <div className="bg-white rounded-[2.5rem] border border-slate-200 p-10 shadow-sm text-left">
                  <div className="flex items-center justify-between mb-8 text-left text-left">
                    <div className="flex items-center gap-4 text-left text-left text-left">
                      ...
                      <div className="text-left text-left text-left text-left">
                        <h3 className="text-xl font-bold text-slate-900 text-left text-left text-left">Analýza Odpovede</h3>
                        <p className="text-slate-400 text-xs font-mono text-left text-left text-left text-left">{discoveryResult.attemptedFullUrl}</p>
                      </div>
                    </div>
                  ...
                  <div className="space-y-4 text-left text-left text-left">
                    <div className="flex items-center gap-2 text-slate-400 text-xs font-black uppercase tracking-tighter text-left text-left text-left text-left text-left text-left text-left text-left text-left"><Code className="w-4 h-4 text-left text-left text-left text-left text-left text-left text-left text-left text-left" /> Raw Output zo servera</div>
                    <div className="bg-slate-900 rounded-3xl p-8 overflow-hidden shadow-2xl border border-slate-800 text-left text-left text-left text-left text-left text-left text-left text-left text-left">
                      {discoveryResult.isHtml ? (
                         <div className="space-y-4 text-left text-left text-left text-left text-left text-left text-left text-left text-left">
                            <div className="flex items-center gap-2 text-rose-400 text-[10px] font-bold uppercase text-left text-left text-left text-left text-left text-left text-left text-left text-left"><Info className="w-3 h-3 text-left" /> IIS Server zamietol kombináciu (Stále 404)</div>
                        ...
                        <pre className="text-emerald-400 text-[11px] font-mono overflow-auto max-h-[500px] leading-relaxed text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center z-50 p-6 animate-in fade-in duration-300">
          <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-lg p-10 space-y-10 animate-in zoom-in-95 text-left text-left text-left text-left text-left text-left">
            <div className="flex items-center justify-between text-left text-left text-left text-left text-left text-left text-left text-left">
              <h3 className="text-2xl font-black text-slate-900 tracking-tight text-left text-left text-left text-left text-left text-left text-left text-left text-left">Nastavenia API</h3>
              <button onClick={() => setShowSettings(false)} className="p-3.5 hover:bg-slate-50 rounded-2xl border border-slate-100 text-slate-400 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left"><X className="text-left text-slate-400" /></button>
            </div>
            <form onSubmit={handleSaveSettings} className="space-y-10 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
              <div className="space-y-5 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
                <label className="block text-xs font-black text-slate-700 uppercase tracking-widest text-left text-left text-left text-left text-left text-left text-left text-left">Flowii API Token</label>
                <input
              ...
              <button type="submit" className="w-full py-5 bg-blue-600 text-white font-black rounded-2xl hover:bg-blue-700 transition-all uppercase text-xs tracking-widest shadow-lg active:scale-95 text-white text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">Uložiť a Synchronizovať</button>
            </form>
          </div>
        </div>
      )}
